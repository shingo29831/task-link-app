...
export const TaskItem: React.FC<Props> = ({ task, projectStartDate, depth, hasChildren, onStatusChange, onDelete, onAddSubTask }) => {
  const config = { 0: { l: '未着手', c: '#888' }, 1: { l: '進行中', c: '#007bff' }, 2: { l: '完了', c: '#28a745' } }[task.status];
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', padding: '10px 0', borderBottom: '1px solid #333', marginLeft: `${depth * 24}px` }}>
      <button 
        ...
        onClick={() => onStatusChange(((task.status + 1) % 3) as 0|1|2)}
      ...
      <div style={{ flex: 1, textAlign: 'left' }}>
        ...
        <span style={{ fontWeight: hasChildren ? 'bold' : 'normal', textDecoration: task.status === 2 ? 'line-through' : 'none', opacity: task.status === 2 ? 0.6 : 1 }}>{task.name}</span>
        {getDeadline()}
      </div>
